...
- 原生主界面已从系统三栏收口为**接近 Tauri 版的信息架构**：左侧 `ProjectSidebarView.swift`、中间主区按状态切换 `MainContentView.swift` / `WorkspaceShellView.swift`、右侧 overlay 抽屉 `ProjectDetailRootView.swift`，并统一走 `NativeTheme.swift` 深色主题；当前交互为 **单击项目打开详情、双击项目进入 workspace 页面**
- 原生 workspace 主线现已进入 **左侧已打开项目列表 + 右侧标签页 / 分屏窗格终端区**：`macos/Sources/DevHavenCore/Models/WorkspaceTopologyModels.swift` 提供 `WorkspaceSessionState / WorkspaceTabState / WorkspacePaneTree / WorkspaceSplitState` 作为单项目 topology 真相源，`OpenWorkspaceSessionState.swift` 负责多项目已打开会话集合，`NativeAppViewModel.swift` 负责已打开项目列表、激活项目切换、关闭回退、**workspace 内继续打开其他项目**，以及每个项目内部标签页/窗格的增删、焦点切换、分屏、缩放、均分与标题同步；其中“返回主列表”现在只会隐藏 workspace 视图，不再清空 `openWorkspaceSessions`，这样回到主列表后再次双击项目仍能恢复原先已打开的项目集合。UI 侧由 `WorkspaceShellView.swift`、`WorkspaceProjectListView.swift`、`WorkspaceProjectPickerView.swift`、`WorkspaceHostView.swift`、`WorkspaceTabBarView.swift`、`WorkspaceSplitTreeView.swift`、`WorkspaceSplitView.swift`、`WorkspaceTerminalPaneView.swift` 组成：左侧项目列表对齐 Tauri 的“已打开项目”，header 上的加号会打开 `WorkspaceProjectPickerView.swift`，把更多项目加入已打开列表；右侧主区则继续递归渲染 Ghostty pane，并通过 `ZStack + ForEach(all open sessions)` 保活所有已打开项目对应的 workspace，只把非激活项隐藏禁交互。当前 Ghostty 主线已按 Supacode 风格拆成五层：`GhosttyAppRuntime.swift`（bundle 资源定位 + 全局 `ghostty_init(...)` + 共享 runtime 入口）、`GhosttyRuntime.swift`（共享 `ghostty_app_t` / `ghostty_config_t` / focus 与 keyboard observer / surface 创建）、`GhosttySurfaceBridge.swift`（Ghostty action callback -> 宿主闭包桥，现已继续桥接 new/goto/move/close tab 与 split action）、`GhosttySurfaceState.swift`（surface 状态）、`GhosttySurfaceView.swift`（AppKit `NSView`、mouse/key 事件、`performKeyEquivalent/flagsChanged/doCommand`、`NSTextInputClient`、preedit、surface config 与 Ghostty 输入 helper）。`GhosttySurfaceHost.swift` 继续保持薄宿主职责：状态条、focus/退出回调和 host model；终端退出时只释放当前终端实例，不再销毁 app 级运行时。
