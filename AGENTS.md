...
- `macos/Sources/DevHavenApp/WorkspaceCommitRootView.swift`
  - Commit tool window 根容器；负责 changes browser / diff preview / commit panel 三分区布局、初始化刷新与分区级结构收口
- `macos/Sources/DevHavenApp/WorkspaceCommitChangesBrowserView.swift`
  - Commit tool window 左侧 changes browser；负责变更列表展示、inclusion toggle、选中项高亮与 change -> diff preview 联动入口
- `macos/Sources/DevHavenApp/WorkspaceCommitDiffPreviewView.swift`
  - Commit tool window 中间 diff preview；负责 loading / empty / error / content 四态展示，不负责 Git diff 读取编排
...
- `NativeAppViewModel.syncActiveWorkspaceToolWindowContext()` 现在同时收口 `prepareActiveWorkspaceCommitViewModel()` 与 `prepareActiveWorkspaceGitViewModel()`；Commit/Git tool window 都要由 ViewModel 统一预热上下文，不要在 App 视图层各自维护缓存
  - Commit tool window 的真实内容由 `WorkspaceCommitRootView` 与其子视图承接；当前主链已覆盖 changes browser、diff preview 与 commit panel 的基础交互，后续高级能力应在既有边界内渐进补齐，不要把提交态散回 `WorkspaceShellView`
