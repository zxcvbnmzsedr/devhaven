...
- 终端右侧快捷命令悬浮窗（可拖拽，按项目记忆位置/开关，支持运行/停止）：`src/components/terminal/TerminalWorkspaceView.tsx`；运行/停止状态统一走 `src/services/quickCommands.ts` + `src-tauri/src/quick_command_manager.rs`；面板状态持久化写入 `terminal_workspaces.json` 的 `workspace.ui.quickCommandsPanel`（类型：`src/models/terminal.ts`；默认/兼容处理：`src/utils/terminalLayout.ts`）
