...
- `macos/Sources/DevHavenApp/WorkspaceGitRootView.swift`
  - Git tool window 根容器；`.log` 时委托 `WorkspaceGitIdeaLogView` 呈现标准 IDEA Log，非 `.log` section 才继续挂载旧 toolbar + sidebar + section 视图
...
- `macos/Sources/DevHavenApp/WorkspaceGitToolbarView.swift`
  - 非 `.log` section 的 Git 搜索、清空筛选与刷新入口
- `macos/Sources/DevHavenApp/WorkspaceGitIdeaLogView.swift`
  - 标准 IDEA Log 根容器；负责 toolbar、log table、bottom pane 与 diff preview 的整体 split 布局
- `macos/Sources/DevHavenApp/WorkspaceGitIdeaLogToolbarView.swift`
  - 标准 IDEA Log filter toolbar；负责搜索、branch/user/date/path filter 与 details/diff preview 开关
...
- `macos/Sources/DevHavenApp/WorkspaceGitIdeaLogBottomPaneView.swift`
  - 标准 IDEA Log 下半区容器；负责 `Changes Browser + Commit Details` 的横向 split
- `macos/Sources/DevHavenApp/WorkspaceGitIdeaLogChangesView.swift`
  - 标准 IDEA Log 左下 changes browser；负责 commit changed files 列表与 selected file 联动
- `macos/Sources/DevHavenApp/WorkspaceGitIdeaLogDetailsView.swift`
  - 标准 IDEA Log 右下 commit details；负责 message / author / refs / parent hashes 呈现
- `macos/Sources/DevHavenApp/WorkspaceGitIdeaLogDiffPreviewView.swift`
  - 标准 IDEA Log 独立 diff preview pane；负责 selected file 的文件级 diff 预览与截断提示
...
- 终端工作区壳：`WorkspaceRootView.swift`、`WorkspaceProjectSidebarHostView.swift`、`WorkspaceChromeContainerView.swift`、`WorkspaceShellView.swift`、`WorkspaceHostView.swift`、`WorkspaceProjectListView.swift`
  ...
  - `WorkspaceShellView` 现在负责 Workspace 中央内容区：上半区是 terminal 主区，下半区是 bottom tool window host；Git tool window 空状态路由、`WorkspaceGitRootView` 挂载、terminal focused search action、pane 快照与 agent signal 接线都在这里收口；tool window 的 `activeKind / isVisible / height` 与 Git section/search/selection 都属于运行时 UI 状态，不进入 workspace restore snapshot。入口按钮不在 Shell 内，而在 `WorkspaceChromeContainerView` 的 stripe 中
  ...
  - Git tool window 的真实内容由 `WorkspaceGitRootView` 与其子视图承接；不要把 Git 业务状态重新塞回 `WorkspaceShellView`
  - 标准 IDEA Log 的 `.log` section 已不再复用 `WorkspaceGitSidebarView`；branch/user/date/path filter 必须在 `WorkspaceGitIdeaLogToolbarView` 内闭环，而不是重新把分支树塞回左栏
