...
- `dev`
  - 本机开发态入口；负责校验 `macos/Vendor`、接入 unified log 并运行 `swift run --package-path macos DevHavenApp`
- `macos/Package.swift`
  - 原生子工程入口
...
- 根目录 `./dev` 是推荐的本机原生开发态入口；它默认会先校验 `macos/Vendor/`，再用 macOS unified log 观察 `DevHavenNative` / `com.mitchellh.ghostty`，最后启动 `swift run --package-path macos DevHavenApp`。
