...
- 终端 UI（xterm、分屏、标签）：主编排 `src/components/terminal/TerminalWorkspaceView.tsx`，顶部栏 `src/components/terminal/TerminalWorkspaceHeader.tsx`，其余终端容器在 `src/components/terminal/*`；`TerminalWorkspaceView` 只接收最小设置 props（如 `sharedScriptsRoot`、`terminalUseWebglRenderer`），避免被全局状态广播拖着重渲染
