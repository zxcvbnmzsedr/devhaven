...
### G. 终端工作区（内置终端 + 布局持久化）
- 终端窗口管理：`src/services/terminalWindow.ts`、`src/components/terminal/TerminalWorkspaceWindow.tsx`；窗口组件仅消费父层显式下发的 `terminalTheme/sharedScriptsRoot/terminalUseWebglRenderer`，不再直接读取全局 Context
...
- 终端工作区 Agent MVP 已改为 **pane 级 pending → shell/agent 双态 + 多 provider adapter**：点击 `src/components/terminal/TerminalTabs.tsx` 的 “+” 只会先创建一个 **pending tab**；pane 本体随后通过 `src/components/terminal/TerminalPendingPane.tsx` 选择 `Shell / Codex / Claude Code / iFlow`，再由 `src/components/terminal/TerminalWorkspaceView.tsx` 把该 pending pane 实化为 shell 或 agent pane。`src/models/terminal.ts` 新增 `pendingTerminal` pane descriptor、`appendPendingTerminalTabToSnapshot`、`realizePendingTerminalPaneInSnapshot`、`splitPendingPaneInSnapshot`；pane 级运行态由 `src/hooks/usePaneAgentRuntime.ts` 管理；provider 适配层位于 `src/agents/registry.ts` 与 `src/agents/adapters/{codex,claudeCode,iflow}.ts`，通用 shell marker 包装位于 `src/agents/shellWrapper.ts`。注意：已定型 `TerminalPane` 不再常驻显示 Agent 控件，也不再提供“新建 Pane”按钮，视觉上尽量保持接近纯终端。
...
- 终端工作区 Agent MVP（**当前支持 Claude Code / Codex / iFlow，但仍不是 durable agent 真相层**）：`src/components/terminal/TerminalWorkspaceView.tsx` 在 `onPtyReady` / 输出流 / session exit 上编排 pane 级 agent 行为，并通过 `src/services/terminal.ts::writeTerminal` 向当前 pane 注入带 marker 的 provider-specific 命令；该能力仍只复用现有 terminal session 与 codex monitor，**不会写入持久恢复真相**，当前也不支持重启自动恢复或 provider-specific resume。
