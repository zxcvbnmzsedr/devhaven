...
- `macos/Sources/DevHavenApp/WorkspaceCommitRootView.swift`
  - Commit tool window 根容器；负责 changes browser / diff preview / commit panel 三分区结构壳与初始化刷新入口
- `macos/Sources/DevHavenApp/WorkspaceCommitChangesBrowserView.swift`
  - Commit tool window 左侧 changes browser 占位视图；负责变更列表容器与后续 inclusion 树承载边界
- `macos/Sources/DevHavenApp/WorkspaceCommitDiffPreviewView.swift`
  - Commit tool window 中间 diff preview 占位视图；负责 preview 头部、空态与后续文件差异内容承载边界
- `macos/Sources/DevHavenApp/WorkspaceCommitPanelView.swift`
  - Commit tool window 底部 panel 占位视图；负责提交信息与后续 options/execution 反馈容器边界
- `macos/Sources/DevHavenApp/WorkspaceGitRootView.swift`
  - Git tool window 根容器；负责顶部 `Git` 标题 + `Log / Console` 次级入口、Console 占位路由，以及 `Log -> WorkspaceGitIdeaLogView / Git -> branches|operations 主链` 的根级切换
...
- 终端工作区壳：`WorkspaceRootView.swift`、`WorkspaceProjectSidebarHostView.swift`、`WorkspaceChromeContainerView.swift`、`WorkspaceShellView.swift`、`WorkspaceHostView.swift`、`WorkspaceProjectListView.swift`
  ...
  - `WorkspaceShellView` 现在负责 Workspace 中央内容区：上半区是 terminal 主区，下半区是 bottom tool window host；Commit/Git tool window 空状态路由、`WorkspaceCommitRootView` / `WorkspaceGitRootView` 挂载、terminal focused search action、pane 快照与 agent signal 接线都在这里收口；当底部 tool window 可见时，terminal 与 tool window 必须通过可拖拽的纵向 `WorkspaceSplitView` 共享高度，拖拽结果写回 `NativeAppViewModel.updateWorkspaceToolWindowHeight(...)`；tool window 的 `activeKind / isVisible / height` 与 Commit/Git 运行时状态都属于内存态，不进入 workspace restore snapshot。入口按钮不在 Shell 内，而在 `WorkspaceChromeContainerView` 的 stripe 中
...
- `NativeAppViewModel.syncActiveWorkspaceToolWindowContext()` 现在同时收口 `prepareActiveWorkspaceCommitViewModel()` 与 `prepareActiveWorkspaceGitViewModel()`；Commit/Git tool window 都要由 ViewModel 统一预热上下文，不要在 App 视图层各自维护缓存
  - Commit tool window 的真实内容由 `WorkspaceCommitRootView` 与其子视图承接；本轮先建立稳定容器边界，后续再把 inclusion tree、preview 交互和 commit options/execution 渐进填充
