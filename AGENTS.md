...
- `macos/Sources/DevHavenApp/WorkspaceModeSwitcherView.swift`
  - Workspace 右侧主内容区一级 `终端 / Git` 模式切换；只负责模式选择，不持有 Git/terminal 业务真相源
- `macos/Sources/DevHavenApp/WorkspaceGitRootView.swift`
  - Git mode 根容器；`.log` 时委托 `WorkspaceGitIdeaLogView` 呈现标准 IDEA Log，非 `.log` section 才继续挂载旧 toolbar + sidebar + section 视图
...
- `macos/Sources/DevHavenCore/Models/WorkspaceGitModels.swift`
  - Workspace Git 面板共享模型、mutation kind 与结构化错误
...
- 终端工作区壳：`WorkspaceShellView.swift`、`WorkspaceHostView.swift`、`WorkspaceProjectListView.swift`
  ...
  - `WorkspaceShellView` 现在同时负责 Workspace 一级 `终端 / Git` 模式切换、Git mode 空状态路由与 `WorkspaceGitRootView` 挂载；Git mode 的 section/search/selection 属于运行时 UI 状态，不进入 workspace restore snapshot
  ...
  - `WorkspaceModeSwitcherView` 只负责 primary mode 切换，不持有第二套 Git/terminal 状态
  - Git mode 的真实内容由 `WorkspaceGitRootView` 与其子视图承接；不要把 Git 业务状态重新塞回 `WorkspaceShellView`
...
- `WorkspaceGitViewModel` 是 Git mode 的 runtime-only 根状态层：`repositoryContext.repositoryPath` 表示仓库级读模型真相源，`selectedExecutionWorktreePath` 表示 worktree 级执行面真相源；其 `logViewModel` 专门承接标准 IDEA Log 的 `.log` section
...
- Git mode 属于 workspace 内的运行时业务面板，不属于 restore snapshot：不要把 Git section、搜索词、selected commit/file、execution worktree、primary mode 持久化到 `WorkspaceRestoreStore`
...
- Workspace Git mode 当前是 runtime-only：`workspacePrimaryMode`、Git section、搜索词、selected commit/file、execution worktree 都不会写入 `WorkspaceRestoreStore` 或 `WorkspaceTerminalRestoreContext`；App 恢复 workspace 时只恢复 terminal pane/context，不恢复 Git 面板当前界面
