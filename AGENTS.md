...
- `macos/Sources/DevHavenApp/WorkspaceGitIdeaLogView.swift`
  - 标准 IDEA Log 根容器；负责 `左侧 branches control strip + 可展开 branches panel + 中间 log table + 最右侧信息栏` 总体布局
...
- `macos/Sources/DevHavenApp/WorkspaceGitIdeaLogChangesView.swift`
  - 标准 IDEA Log 右侧信息栏上半区 changes browser；负责 commit changed files 列表、`文件名主标题 + 路径/rename 次信息` 分层展示与 selected file 联动
