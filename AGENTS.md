...
- Ghostty 集成：`Ghostty/` 目录下的 runtime / surface / host / view 相关文件
  ...
  - `GhosttyRuntimeEnvironmentBuilder` / `GhosttySurfaceHostModel` 负责把 `DEVHAVEN_AGENT_SIGNAL_DIR`、`DEVHAVEN_AGENT_RESOURCES_DIR` 与 wrapper `PATH` 注入内嵌终端；终端环境注入在 host 层收口，不要把 bundle 解析散落到 view / script / ViewModel 多处
...
- 为 DevHaven 在既有工作区通知体验之上补齐 Claude / Codex Agent 会话状态感知：内嵌终端可通过 wrapper / hooks / signal store 让侧边栏直接感知“正在运行 / 等待处理 / 已完成 / 失败”，而不是继续依赖离散通知事件猜测会话状态。
- 修复交互式 Codex “当前回合已完成但会话仍存活”时仍显示“正在运行”的语义错位，并进一步降低长输出场景下的误判：保留 signal 进程态主链，引入 Codex 官方 notify 作为 waiting 主判据，再由 App 层活动度 / 可见文本做最小 fallback 修正。
