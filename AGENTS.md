...
- `macos/Sources/DevHavenApp/WorkspaceCommitSideToolWindowHostView.swift`
  - Commit 左侧独立工具窗宿主；负责 Commit 空态 / `WorkspaceCommitRootView` 路由、侧边焦点桥接，以及把 changes browser 双击统一桥接到 workspace 独立 diff 标签页
- `macos/Sources/DevHavenApp/WorkspaceCommitRootView.swift`
  - Commit tool window 根容器；当前负责 changes browser + commit panel 双分区布局、初始化刷新，以及统一 open diff 闭包向下传递
- `macos/Sources/DevHavenApp/WorkspaceCommitChangesBrowserView.swift`
  - Commit tool window 左侧 changes browser；负责变更列表展示、inclusion toggle、选中项高亮与双击打开独立 diff 标签页；单击仍只维护 selection / preview 语义
...
- `macos/Sources/DevHavenApp/WorkspaceDiffTabView.swift`
  - Workspace 独立 diff 标签页视图；负责 side-by-side / unified viewer toolbar、加载/空态/错误态，以及解析后 diff 文档呈现
...
- `macos/Sources/DevHavenCore/Models/WorkspaceDiffModels.swift`
  - Workspace runtime diff 标签页共享模型；负责 diff source/open request/presented tab/document state，以及“打开来源上下文（origin presented tab / focused area）”等统一表达
...
- `macos/Sources/DevHavenCore/Storage/WorkspaceDiffPatchParser.swift`
  - Workspace diff patch parser；负责把 unified diff 解析成统一文档模型与 side-by-side 行结构
...
- `macos/Sources/DevHavenCore/ViewModels/WorkspaceDiffTabViewModel.swift`
  - Workspace 独立 diff 标签页运行时状态层；负责按 source 加载 Git Log / working tree diff、维护 viewer mode 与文档加载态
...
- 终端工作区壳：`WorkspaceRootView.swift`、`WorkspaceProjectSidebarHostView.swift`、`WorkspaceChromeContainerView.swift`、`WorkspaceShellView.swift`、`WorkspaceHostView.swift`、`WorkspaceProjectListView.swift`
  ...
  - `WorkspaceShellView` 也是 Git Log -> workspace diff 标签页的 App 层入口：Git Log changes browser 的双击只能统一收口到 `NativeAppViewModel.openActiveWorkspaceDiffTab(...)`，不要在 Git tool window 内再做第二套 preview 弹层或独立窗口状态
...
- `NativeAppViewModel.syncActiveWorkspaceToolWindowContext()` 现在同时收口“可见的侧边 Commit”和“可见的底部 Git”两条上下文预热：`prepareActiveWorkspaceCommitViewModel()` 与 `prepareActiveWorkspaceGitViewModel()` 都要由 ViewModel 统一驱动，不要在 App 视图层各自维护缓存
  ...
  - Commit changes browser 的双击打开逻辑必须与 Git Log 共用 `NativeAppViewModel.openActiveWorkspaceDiffTab(...)` 主链，source 使用 working tree diff；不要在 Commit tool window 内额外分叉一套“临时 diff”状态
...
- `NativeAppViewModel` 还负责按当前 active workspace session 管理 `workspaceDiffTabsByProjectPath / workspaceSelectedPresentedTabByProjectPath` 等 runtime diff tab 状态；统一入口是 `openActiveWorkspaceDiffTab(...) -> openWorkspaceDiffTab(...)`，不要让 Git Log / Commit 各自维护 projectPath 归属或第二套 tab registry
