...
- `macos/Sources/DevHavenApp/WorkspaceRunToolbarView.swift`
  - workspace 顶部右侧的轻量运行控制区；负责展示当前项目 `Project.scripts` 运行配置菜单，以及 Run / Stop / Logs / 配置按钮，不直接持有进程或日志真相源
...
- `macos/Sources/DevHavenApp/WorkspaceScriptConfigurationSheet.swift`
  - 当前项目的脚本配置面板；负责编辑 `Project.scripts`，并在配置阶段提供“插入通用脚本（可选）”入口，把 `sharedScriptsRoot` 下的通用脚本当作模板源，而不是直接运行项
...
- `macos/Sources/DevHavenCore/Models/WorkspaceRunModels.swift`
  - workspace run configuration / reused session / console state / manager event 模型；只描述运行配置、按配置复用的会话槽位与底部 console 展示态，不直接执行命令
...
- 工作区运行体验：`WorkspaceRunToolbarView.swift`、`WorkspaceRunConsolePanel.swift`、`WorkspaceScriptConfigurationSheet.swift`
  - `WorkspaceHostView` 负责把当前 workspace/project 的 run state 接到顶部 `WorkspaceRunToolbarView` 与底部 `WorkspaceRunConsolePanel`；顶部 `配置` 入口应打开当前项目脚本配置面板，需要管理通用脚本时再由该面板跳转到设置页的脚本分类；App 层只做按钮桥接与日志展示，不要在 SwiftUI 里直接 new `Process`
...
- workspace Run Console 的运行配置来源固定由当前项目 `Project.scripts` 推导；`sharedScriptsRoot` 下的通用脚本只在 `WorkspaceScriptConfigurationSheet` 中作为脚本模板源使用，不直接出现在运行菜单里；同一配置重复 Run 时必须复用同一个 console tab，而不是无限累积 execution history；真正的执行与停止必须经由 `WorkspaceRunManager`，不要让 `WorkspaceRunToolbarView` / `WorkspaceRunConsolePanel` 自己直接持有 `Process`
