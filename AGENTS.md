...
- 原生 workspace 主线现已进入 **左侧已打开项目列表 + 右侧标签页 / 分屏窗格终端区**，并继续补齐了 **worktree 原生闭环**：`macos/Sources/DevHavenCore/Models/WorkspaceTopologyModels.swift` 继续提供 `WorkspaceSessionState / WorkspaceTabState / WorkspacePaneTree / WorkspaceSplitState` 这套**projection 模型**，但项目内终端布局 owner 已从全局 ViewModel 收口为 `macos/Sources/DevHavenCore/ViewModels/GhosttyWorkspaceController.swift`。`OpenWorkspaceSessionState.swift` 现在只负责多项目已打开会话集合与 controller 绑定，`NativeAppViewModel.swift` 只负责已打开项目列表、激活项目切换、关闭回退、**workspace 内继续打开其他项目 / worktree**、系统终端打开动作、原生 Git/worktree service 调度、`Project.worktrees` 持久化与全局 worktree 进度态派生；每个项目内部标签页/窗格的增删、焦点切换、分屏、缩放、均分与标题同步则统一交给 `GhosttyWorkspaceController`。其中“返回主列表”现在只会隐藏 workspace 视图，不再清空 `openWorkspaceSessions`，这样回到主列表后再次双击项目仍能恢复原先已打开的项目集合。UI 侧由 `WorkspaceChromePolicy.swift`、`WorkspaceShellView.swift`、`WorkspaceProjectListView.swift`、`WorkspaceProjectPickerView.swift`、`WorkspaceWorktreeDialogView.swift`、`WorktreeInteractionOverlayView.swift`、`WorkspaceHostView.swift`、`WorkspaceTabBarView.swift`、`WorkspaceSplitTreeView.swift`、`WorkspaceSplitTreeViewKeyPolicy.swift`、`WorkspaceSplitView.swift`、`WorkspaceTerminalPaneView.swift` 组成：workspace 左侧仍保留“已打开项目”列表用于切换/关闭/继续打开项目，现在同一父项目下还会展示 worktree 子项，并支持刷新、创建/打开已有、失败重试与删除；右侧终端区则继续按极简模式收口——`WorkspaceHostView.swift` 不再显示项目名/路径/统计/“系统终端/查看详情”header，`WorkspaceTerminalPaneView.swift` 也不再显示 pane 顶部工具条，`GhosttySurfaceHost.swift` 也不再显示运行态 status/title/pwd 顶部 pill，默认直接展示 Ghostty 终端内容。`WorkspaceSplitTreeViewKeyPolicy.swift` 进一步把 split subtree 的挂载边界写死：当前 DevHaven 原生 workspace 的真实 Ghostty surface owner 仍在 `WorkspaceTerminalSessionStore` 外部 store 中，因此 root / split subtree 默认不能按 structural identity 强制 remount，只保留 leaf pane 的稳定 identity，避免单 pane -> 双 pane 时把已有 pane 内容整块重建。标签页标题则继续由 projection 侧稳定生成：`WorkspaceTabTitlePolicy.swift` 负责默认 `终端1/终端2/...` 命名，并忽略 shell runtime 回传的路径/prompt 标题，避免 tab 再次被路径覆盖；右侧主区继续通过 `ZStack + ForEach(all open sessions)` 保活所有已打开项目与 worktree 对应的 workspace，只把非激活项隐藏禁交互。为避免 split/tree 重排、host 重组或 active project 切换时旧 pane 再次被误释放，terminal owner 又从 host-local 继续上提到 **workspace shell 级**：`WorkspaceShellView.swift` 现持有 `WorkspaceTerminalStoreRegistry`，按 `projectPath` 复用 `WorkspaceTerminalSessionStore`；`WorkspaceTerminalSessionStore.swift` 内部按 `pane.id` 复用 `GhosttySurfaceHostModel`，`WorkspaceHostView.swift` 只消费对应 store，并通过 `syncRetainedPaneIDs(...)` 同步真实保留的 pane 集合。`WorkspaceShellView.swift` 还会在 `.onAppear`、激活项目切换、`activeWorkspaceLaunchRequest?.paneId` 变化和 open sessions 变化时，对当前激活项目的 selected pane 做 eager warm-up，使“已有 workspace 壳已出现，但 shell 还在初始化”的阶段更接近 Ghostty / Supacode 的呈现边界。当前还补了一条专门给“进入命令行慢”排查用的诊断链：`WorkspaceLaunchDiagnostics.swift` 会在 `NativeAppViewModel.enterWorkspace(_:)` 记录目标项目已有多少 tab/pane、当前 open sessions 总量，在 `WorkspaceShellView.swift` / `WorkspaceHostView.swift` 记录 shell/host 挂载，在 `GhosttySurfaceHost.swift` 记录 surface 新建/复用与耗时；排查这类问题时，应优先先看这组日志，而不是只凭体感猜是 cwd、shell 还是 workspace topology。worktree 相关本地能力则统一收口到 `macos/Sources/DevHavenCore/Models/NativeWorktreeModels.swift` 与 `macos/Sources/DevHavenCore/Storage/NativeGitWorktreeService.swift`：前者定义原生 branch/worktree 列表、创建/删除请求、进度步骤与 sidebar/overlay projection；后者直接执行 `git branch --list`、`git worktree list/add/remove`、`git branch -d`，默认目标目录固定为 `~/.devhaven/worktrees/<project>/<branch>`，创建新 worktree 时会尝试复制主仓库 `.devhaven` 并执行 `.devhaven/config.json` 的 `setup` 命令（失败只回 warning，不阻断创建）。当前 Ghostty 主线已按 Supacode 风格拆成五层：`GhosttyAppRuntime.swift`（bundle 资源定位 + 全局 `ghostty_init(...)` + 共享 runtime 入口）、`GhosttyRuntime.swift`（共享 `ghostty_app_t` / `ghostty_config_t` / focus 与 keyboard observer / surface 创建）、`GhosttySurfaceBridge.swift`（Ghostty action callback -> 宿主闭包桥，现已继续桥接 new/goto/move/close tab 与 split action，并补接 `GHOSTTY_ACTION_SCROLLBAR`）、`GhosttySurfaceState.swift`（surface 状态）、`GhosttySurfaceView.swift`（AppKit `NSView`、mouse/key 事件、`performKeyEquivalent/flagsChanged/doCommand`、`NSTextInputClient`、preedit、surface config、occlusion、scroll wrapper 接口与 Ghostty 输入 helper）。`GhosttySurfaceHost.swift` 继续保持薄宿主职责：运行时 focus/退出回调与 host model 管理；终端退出时只释放当前终端实例，不再销毁 app 级运行时。为了实现“pane 先可见，shell 后就绪”，`GhosttySurfaceHostModel` 新增 `hasPreparedSurfaceView`，`GhosttySurfaceHost.swift` 会按 `WorkspaceTerminalStartupPresentationPolicy.swift` 在 surface 还没 ready、但初始化尚未失败时显示“正在启动 shell...” overlay，而不是去解析 prompt 文本。2026-03-20 当天又继续按 supacode 的 terminal 内核边界收口了四轮：`WorkspacePaneTree` / `Node` 现已补齐 **structural identity（显式忽略 split ratio）**，`WorkspaceSplitTreeView.swift` 用这层稳定身份约束 subtree 重建；`GhosttyTerminalView.swift` 的 `NSViewRepresentable.updateNSView(...)` 也已改成默认 no-op，不再把普通 SwiftUI update 主动放大成 terminal host sync，pane/tab 焦点切换改由 `isFocused` 变化显式驱动。随后先补了 `GhosttySurfaceContainerView.swift` 这层稳定 AppKit 容器与 `GhosttySurfaceView.swift` 里的 `focusDidChange / moveFocus` 风格焦点同步，再补了 `GhosttySurfaceResizePolicy.swift`，让 `GhosttySurfaceView.swift` 只在 backing size 真变化且当前网格仍满足最小尺寸时才执行 `ghostty_surface_set_size(...)`，并在 backing properties 变化时同步 `contentsScale`、关闭 layer 隐式动画。最新又继续直接照抄 supacode 的 window activity / occlusion / scroll wrapper 主线：新增 `WindowFocusObserverView.swift` 监听 `didBecomeKey / didResignKey / didChangeOcclusionState`，新增 `WorkspaceSurfaceActivityPolicy.swift` 统一把 workspace/tab/window 的可见性与焦点映射成 surface activity，`GhosttyTerminalView.swift` 已改走 `GhosttySurfaceScrollView.swift` 作为 representable 根节点，`WorkspaceHostView.swift` 会在每次 body 更新时把 active workspace、selected tab、focused pane 与 window activity 显式同步给各个 `GhosttySurfaceHostModel`，再由 host model 把 `setOcclusion(...) / focusDidChange(...)` 应用到真实 `GhosttySurfaceView`。同时 `WorkspaceSplitView.swift` 也已改成更接近 supacode 的 split wrapper 实现。
...
- 原生 App 本地打包脚本：`macos/scripts/build-native-app.sh`（只负责把 `swift build` 产物封装为本地 `.app`、完成 ad-hoc 签名，并输出到 `macos/.build/native-app/<debug|release>/` 这类稳定目录；默认只 `open` 产物目录，不写入 `/Applications`）
...
- 测试：`swift test --package-path macos`；当前与原生终端主线直接相关的测试位于 `macos/Tests/DevHavenCoreTests/{GhosttyWorkspaceControllerTests,NativeAppViewModelWorkspaceEntryTests,WorkspaceLaunchDiagnosticsTests,WorkspaceSubsystemTests,WorkspaceTopologyTests}.swift` 与 `macos/Tests/DevHavenAppTests/{AppRootContentVisibilityPolicyTests,GhosttyAppRuntimeTests,GhosttySharedRuntimeTests,GhosttySurfaceBridgeTabPaneTests,GhosttySurfaceHostTests,GhosttySurfaceContainerViewTests,GhosttySurfaceFocusRequestPolicyTests,GhosttySurfaceRepresentableUpdatePolicyTests,GhosttySurfaceResizePolicyTests,GhosttySurfaceScrollViewTests,InitialWindowActivatorTests,WorkspaceChromePolicyTests,WorkspaceSurfaceActivityPolicyTests,WorkspaceSurfaceRegistryTests,WorkspaceTerminalSessionStoreTests,WorkspaceTerminalStoreRegistryTests,WorkspaceTerminalStartupPresentationPolicyTests}.swift`。其中 `GhosttySurfaceHostTests` 现会在 xctest 宿主下 `ghostty_surface_new(...)` 直接失败时自动 skip 交互 smoke，并继续断言错误路径被正确暴露。
- 当前边界：此子工程现在已经覆盖 **workspace 主路径 + 左侧已打开项目列表 + 右侧标签页 / 分屏窗格终端区 + 父项目下嵌套 worktree 子项与原生 Git worktree 创建/打开已有/删除/重试/刷新闭环**，支持多项目切换、多标签页、多窗格、Ghostty action 驱动的标签页/分屏操作，以及项目切换/标签页切换时对非激活 workspace 和非选中 tab 的保活挂载；同时已经补齐了**workspace 内继续打开其他项目 / worktree**的入口，不再需要先退出当前 workspace 才能把第二个项目加入左侧列表。当前仍**不覆盖布局持久化、PTY 持久化 / 重放、control plane 原生投影、运行配置与文件/Git 侧边栏**。当前 workspace 壳默认只保留左侧已打开项目列表（含 worktree 子层级）与右侧 Ghostty 终端主区，首页目录 sidebar、项目 header 和 pane 工具条都不会在 workspace 内继续常驻；若后续还要恢复这些动作，必须先确认它们属于 workspace 壳层入口还是终端区入口，避免再次把纯终端区做回信息面板。
