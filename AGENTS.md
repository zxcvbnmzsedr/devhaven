...
- 项目快捷命令（配置/编辑/删除/运行/停止）：`src/components/DetailPanel.tsx`（入口）→ `src/App.tsx`（打开终端并派发事件）→ `src/services/terminalQuickCommands.ts` → `src/components/terminal/TerminalWorkspaceView.tsx`（执行）；不再配置“停止命令”文本，停止为运行态终止；持久化在 `projects.json`（字段：`Project.scripts`，模型：`src/models/types.ts`，当前仅 `name/start/paramSchema/templateParams`）
