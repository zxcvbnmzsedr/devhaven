...
- `macos/Sources/DevHavenApp/WorkspaceCommitSideToolWindowHostView.swift`
  - Commit 左侧独立工具窗宿主；负责 Commit 空态 / `WorkspaceCommitRootView` 路由、侧边焦点桥接，以及“Commit 属于左侧独立 tool window”这一层级收口
- `macos/Sources/DevHavenApp/WorkspaceCommitRootView.swift`
  - Commit tool window 根容器；当前负责 changes browser + commit panel 双分区布局、初始化刷新与分区级结构收口
